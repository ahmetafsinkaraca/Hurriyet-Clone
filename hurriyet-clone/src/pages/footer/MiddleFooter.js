...
export default function MiddleFooter() {
  return (
    <div className='px-[15rem] flex items-center flex-col '>

      <div className='flex items-center p-4 pt-8 gap-x-10 w-[65rem]'>
        ...
        <div className='flex gap-x-10 pl-[8rem]'>
          <NavLink className='border p-3 rounded-full border-gray-500 flex items-center justify-center'>
            <FaFacebookF />
          </NavLink>
      ...
      <div className="flex justify-center items-center w-full py-2 flex-shrink-0">
        <div className="w-[70rem] h-0.5 bg-gray-400 opacity-30"></div>
      </div>

      <div className='flex gap-x-10 p-2 text-nowrap'>

        <div className='flex flex-col'>
          {footer && footer.slice(0, 9).map((footers, id) => (
            <NavLink to={`/details/${footers.id}`}>
              <soan className='font-bold text-[13px]'>{footers.title}</soan>
            </NavLink>
          ))}
        </div>
