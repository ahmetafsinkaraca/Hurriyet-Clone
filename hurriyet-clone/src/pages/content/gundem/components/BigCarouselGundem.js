...
export default function BigCarouselGundem() {
                    ...
                    if (index < itemsPerPageOther) {
                        return (
                            <div key={index} className='flex flex-col items-center justify-center'>
                                <NavLink to={`/gundem/details/${gundems.id}`} className='flex flex-col'>
                                    <img src={gundems.image} className='w-[53.5rem] hover:opacity-70 transition duration-300 rounded' />
                                    <span className='font-bold text-[28px] hover:opacity-70 transition duration-300'>{gundems.title}</span>
                                    <span className='w-[54rem] py-3 hover:opacity-70 transition duration-300'>{gundems.description}</span>
                        ...
                        return (
                            <NavLink key={index} to={`details/${gundems.id}`} className='flex flex-col'>
                                <div className='flex gap-x-5 py-10'>
                                    <img src={gundems.image} className='w-[30rem] hover:opacity-70 transition duration-300 rounded' />
                                    <div className='flex flex-col'>
                                        <span className='w-[25rem] font-bold text-3xl hover:opacity-70 transition duration-300'>{gundems.title}</span>
                                        <span className='w-[25rem] py-3 hover:opacity-70 transition duration-300 text-[17px] font-light'>{gundems.description}</span>
