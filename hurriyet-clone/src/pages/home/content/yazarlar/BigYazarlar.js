...
export default function BigYazarlar() {
  return (
    <div className='flex flex-col items-center justify-center mt-5 h-full'>
        ...
        >
          {yazarlar && yazarlar.map((yazar, id) => (
            <SwiperSlide key={yazar.id}
              className="swiperSlideCustom flex items-center opacity-100 hover:opacity-70 transition-all duration-100">
              ...
              <NavLink to={`/detail/${yazar.id}`} className="flex flex-col items-center">
                ...
                <div className='basis-auto flex-nowrap whitespace-nowrap hover:text-opacity-60'>
                  <img src={yazar.image} className='w-[11.5rem] h-[9.5rem]' />
                </div>
                <div className='bg-white w-[11.5rem]'>
                  <div className='p-4 flex flex-col line-clamp-3'>
                    <span className='font-bold text-red-600 text-[15px] hover:text-opacity-60 bg-white]'>{yazar.name}</span>
                    <span className='text-md text-left text-black font-bold hover:text-opacity-60 bg-white pt-3 pb-4'>{yazar.description}</span>
                  </div>
                </div>
      ...
      <div className='-translate-x-[3rem] -translate-y-7 z-20'>
        <div className=' flex items-center justify-center'>
          <button>
            <GrPrevious className=' swiper-button-prev-other w-6 h-6 rounded-full text-black' />
          </button>
        </div>
      </div>
      <div className='translate-x-[3rem] -translate-y-[3.3rem] z-20'>
        <div className=' flex items-center justify-center'>
          <button>
            <GrNext className=' swiper-button-next-other w-6 h-6 rounded-full text-black ' />
          </button>
        </div>
      </div>
