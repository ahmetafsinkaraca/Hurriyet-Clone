...
import gundem from './gundem.json'

export const ProductCard = ({ title, image, updateDate, description, hastag }) => {
    return (
            ...
            <div className='flex justify-center text-wrap gap-x-[3.5rem]'>
                ...
                <div className='flex flex-col pt-8'>
                    ...
                    <div className='flex text-xs font-bold flex-shrink-0 items-start justify-start'>
                        <div><span className='hover:opacity-70 transiton duration-300 cursor-pointer'>HABERLER</span></div>
                        <MdNavigateNext className='items-center justify-center flex' />
                        <div><span className='hover:opacity-70 transiton duration-300 cursor-pointer'>GÜNDEM</span></div>
                    </div>
                    ...
                    <div className='flex flex-col pt-4 gap-x-2'>
                        ...
                        <span className='py-4'>Güncelleme Tarihi: {updateDate}</span>
                            ...
                            <span className='flex gap-x-[4rem] absolute top-[1.6rem] left-4'>
                                ...
                                <span className='translate-x-[50rem]'>
                                    <div className='flex gap-x-2 items-center justify-center'>
                                        <div className='flex items-center justify-center w-8 h-8 border p-2 rounded-full border-gray-500 bg-[#2f619d] hover:bg-gray-800 hover:opacity-90 transition duration-300'>
                                            <FaFacebookF className='text-white' />
                                        </div>
                                        <div className='flex items-center justify-center w-8 h-8 border rounded-full border-gray-500 hover:bg-zinc-100 transition duration-300'>
                    ...
                    <div className='flex gap-x-12'>
                        ...
                        <div className='py-10 flex flex-col pt-14'>
                            <img src={image} className='w-[58rem] h-[33rem]' />

                            <div className='flex justify-between w-[52rem] py-2'>
                                <div className='font-semibold'>İHA</div>

                                <div className='flex gap-x-2'>
                                    <div className='text-sm translate-y-1'>Oluşturulma Tarihi: {updateDate}</div>
                        ...
                        <div className='relative py-10 flex flex-col pt-20 gap-y-6'>
                            ...
                            {Array.from({ length: 3 }).map((_, index) => (
                                <div>
                                    <img src={image} className='w-[18rem] h-[11rem] object-cover' />
                                    <div className='second flex flex-col border px-2 items-center justify-center'>
                                        <span className='w-[16rem] h-[8rem] py-4 font-bold text-lg'>{truncateText(title, 48)}</span>
                                    </div>
                                </div>
                            ))}
                            ...
                            <div className='absolute flex flex-col gap-y-[19rem] px-5 py-[20rem]'>
                                {Array.from({ length: 3 }).map((_, index) => (
                                    <div >
                                        <span className='text-red-600 font-bold text-[11px]'>{hastag}</span>
                                    </div>
                                ))}
                            </div>
